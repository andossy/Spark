...
P_vec = [[0.5e-4, 0.937]]#, [2.5e-4, 0.915]]

base_size = 6
f=pylab.figure(figsize=(base_size*2, base_size*len(P_vec)))
pylab.interactive(False)
for ind, (P_nucleation, P_retention) in enumerate(P_vec):
    ...
    pylab.subplot(len(P_vec), 2 ,1+ind*len(P_vec))
    ...
    pylab.figure(f.number)
    pylab.subplot(len(P_vec), 2, 2+ind*(len(P_vec)))
    pylab.imshow(csqn_data, interpolation="nearest", origin="lower",\
                 extent=[0,N*0.030,0,N*0.030], cmap=cmap)
    title = "CSQN distribution"
    if ind:
        title += " (failing)"
    pylab.title(title)
    pylab.xlabel("$\mu$m")
    ...
    failing = "" if not ind else "_failing"
    data.nonzero()[0].tofile(open("i_RyR_indices{}.dat".format(failing), "w"), sep="\n")
    data.nonzero()[1].tofile(open("j_RyR_indices{}.dat".format(failing), "w"), sep="\n")
    csqn_data.nonzero()[0].tofile(open("i_csqn_indices{}.dat".format(failing), "w"), sep="\n")
    csqn_data.nonzero()[1].tofile(open("j_csqn_indices{}.dat".format(failing), "w"), sep="\n")
    #pylab.draw()
...
f.savefig("cluster_distribution.pdf")
